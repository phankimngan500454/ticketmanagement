...
class _WebManagerDashboardState extends State<WebManagerDashboard> {
  ...
  Future<void> _loadData() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _feedbacks = List.from(feedbacks)
          ...
          if (_selectedTicket != null) {
            try {
              _selectedTicket = _feedbacks.firstWhere(
  ...
  List<Ticket> get _filtered {
    ...
    return _feedbacks.where((t) {
      ...
      if (_typeFilter == 'reopen_medical' && t.ticketType != 'reopen_medical')
        return false;
      if (_typeFilter == 'reopen_medical_ins' &&
          t.ticketType != 'reopen_medical')
        return false;
      if (_typeFilter == 'reopen_medical_fin' &&
          (t.ticketType != 'reopen_medical' ||
              !t.description.toLowerCase().contains('ảnh hưởng tài chính: có')))
        return false;

      final matchStatus =
          _filterStatus == 'Tất cả' || t.status == _filterStatus;
      final matchDept =
          _deptFilter == null || t.requesterDeptName == _deptFilter;
      final matchSearch =
          q.isEmpty ||
                      ...
                      child: Row(
                        children: [
                          ...
                          _statusChip(
                            ...
                            _typeFilter == 'reopen_medical' ||
                                    _typeFilter == 'reopen_medical_ins' ||
                                    _typeFilter == 'reopen_medical_fin'
                                ? 'Chờ duyệt'
                                : 'Chưa xử lý',
                          ),
                          _statusChip(
                    ...
                    children: [
                      Row(
                        children: [
                          Container(
                            ...
                            child: Text(
                              isReopen
                                  ? '#BA-${t.ticketId.toString().padLeft(4, '0')}'
                                  : '#GY-${t.ticketId.toString().padLeft(4, '0')}',
                              style: TextStyle(
                      ...
                      if (isReopen) ...[
                        Builder(
                          builder: (context) {
                            final desc = t.description;
                            final isFinance = desc.toLowerCase().contains(
                              'ảnh hưởng tài chính: có',
                            );
                      ...
                      Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              '${t.requesterName ?? ''} - ${t.requesterDeptName ?? ""}',
                              style: TextStyle(
                                fontSize: 11,
        ...
        children: [
          ...
          Expanded(
            child: TicketDetailWrapper(
              key: ValueKey(
                'detail_mgr_${_selectedTicket!.ticketId}_${_selectedTicket!.status}',
              ),
