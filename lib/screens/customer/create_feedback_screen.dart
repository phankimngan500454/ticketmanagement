...
class CreateFeedbackScreen extends StatefulWidget {
  ...
  const CreateFeedbackScreen({super.key, required this.currentUser, required this.onTypeChanged});
...
class _CreateFeedbackScreenState extends State<CreateFeedbackScreen> {
      ...
      if (f.size > 5 * 1024 * 1024) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('⚠️ ${f.name} vượt quá 5MB'), backgroundColor: Colors.orange, behavior: SnackBarBehavior.floating),
          );
        }
        continue;
      }
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('✅ Gửi góp ý thành công!'), backgroundColor: Color(0xFF43A047), behavior: SnackBarBehavior.floating),
        );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('❌ Có lỗi xảy ra, vui lòng thử lại!'), backgroundColor: Colors.redAccent, behavior: SnackBarBehavior.floating),
        );
      }
    } finally {
                ...
                child: Row(children: [
                  ...
                  Container(
                    ...
                    child: const Row(mainAxisSize: MainAxisSize.min, children: [
                      Icon(Icons.feedback_rounded, size: 14, color: Colors.white70),
                      SizedBox(width: 4),
                      Text('💬 Góp ý', style: TextStyle(fontSize: 11, color: Colors.white)),
                    ]),
                  ),
                ]),
              ),
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                _card(
                  ...
                  child: TextFormField(
                    ...
                    decoration: _inputDeco('Nhập tiêu đề ngắn gọn...'),
                ...
                _card(
                  ...
                  child: TextFormField(
                    ...
                    maxLines: 6,
                    decoration: _inputDeco('Mô tả chi tiết góp ý, đề xuất cải tiến...'),
            ...
            itemBuilder: (_, i) {
              ...
              return Stack(clipBehavior: Clip.none, children: [
                Container(
                  ...
                  child: isImg
                    ? ClipRRect(borderRadius: BorderRadius.circular(10), child: Image.memory(pf.bytes, fit: BoxFit.cover))
                    : Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                        const Icon(Icons.insert_drive_file_rounded, color: _teal, size: 28),
                        const SizedBox(height: 4),
                        Padding(padding: const EdgeInsets.symmetric(horizontal: 4),
                          child: Text(pf.name, style: const TextStyle(fontSize: 8), maxLines: 2, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center)),
                      ]),
                ),
      ...
      GestureDetector(
        ...
        child: Container(
          ...
          child: const Column(children: [
            Icon(Icons.add_photo_alternate_rounded, size: 28, color: _teal),
            SizedBox(height: 6),
            Text('Chọn ảnh hoặc file', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: _teal)),
            SizedBox(height: 2),
            Text('JPG · PNG · GIF · PDF · DOC — tối đa 5MB/file', style: TextStyle(fontSize: 10, color: Color(0xFF888888))),
          ]),
        ),
      ),
    ]),
  ...
  Widget _buildSubmitButton() => Container(
    ...
    decoration: BoxDecoration(
      gradient: const LinearGradient(colors: [_tealDark, _teal]),
    ...
    child: ElevatedButton.icon(
      ...
      label: Text(_submitting ? 'Đang gửi...' : 'GỬI GÓP Ý',
  ...
  Widget _card({required IconData icon, required Color iconColor, required String label, required Widget child, bool required = false, String? badge}) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Text(label, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF1C1C2E))),
          if (required) ...[
            const SizedBox(width: 4),
            const Text('*', style: TextStyle(color: Color(0xFFE53935), fontWeight: FontWeight.bold, fontSize: 16)),
          ],
          if (badge != null) ...[
            const SizedBox(width: 8),
            Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
              decoration: BoxDecoration(color: Colors.grey[100], borderRadius: BorderRadius.circular(6)),
              child: Text(badge, style: TextStyle(fontSize: 9, color: Colors.grey[500], fontWeight: FontWeight.bold))),
          ],
        ]),
